...
const addGet = async () => {
  try {
    if (user.value.role == 'admin') {
      ...
      const data = await MFA.addGet()
      router.push(`maintenance-facility/${data.id_mfa}?isAdd=true`)
    }
...
const deletePost = (id_mfa) => {
  if (user.value.role == 'admin') {
    MFA.removeItem(id_mfa).then(() => {
...
watch(page_number, async (e) => {
  try {
    isLoading.value = true
    await MFA.readItem(search_type.value, search_data.value, e, row_per_page.value)
    isLoading.value = false
  } catch (error) {
    isLoading.value = false
...
watch(row_per_page, async (e) => {
  try {
    ...
    const data = await MFA.readItem(search_type.value, search_data.value, page_number.value, e)
...
watch(search_data, async (e) => {
  try {
    isLoading.value = true
    const data = await MFA.readItem(search_type.value, e, page_number.value, row_per_page.value)
    total_pages.value = data
    isLoading.value = false
  } catch (error) {
    isLoading.value = false
...
watch(search_type, async (e) => {
  try {
    isLoading.value = true
    const data = await MFA.readItem(e, search_data.value, page_number.value, row_per_page.value)
    total_pages.value = data
    isLoading.value = false
  } catch (error) {
    isLoading.value = false
...
<template>
  ...
  <div class="px-5 lg:px-48 rounded-lg text-sm lg:text-base pb-[20vh]">
    ...
    >
      <div class="w-10/12 lg:w-11/12">
        <div class="grid grid-cols-12 text-black px-6 py-2 items-center">
          <div class="col-span-12 lg:col-span-8">
            <span class="text-sm">ID: {{ item.id_mfa }}</span>
          ...
          >
            <div class="flex justify-center">
              <RouterLink
                :to="`maintenance-facility/${item.id_mfa}?isEdit=true`"
      ...
      <RouterLink
        :to="`maintenance-facility/${item.id_mfa}`"
    ...
    <ModalBody class="p-0">
      <div class="p-5 text-center">
        ...
        <div class="text-lg mt-5">
          ...
          <b> {{ maintenance_facilities.id_mfa }}</b> ?
        </div>
      </div>
      <div class="px-5 pb-8 text-center">
        ...
        <button
          ...
          @click="deletePost(maintenance_facilities.id_mfa)"
        >
          Delete
        </button>
      </div>
